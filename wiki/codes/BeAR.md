...
- [[2025_Schmidt_K2-18b]] — used as the **independent cross-check** retrieval against [[POSEIDON]] for K2-18 b's NIRISS+NIRSpec data. Applied to 4 of the 60 data combinations (FIREFLy + exoTEDRF × R≈100 + pixel-level); detection significances 3.5–4.5σ for CH₄ and Bayes factors < 3 for CO₂/DMS — in "excellent agreement" with POSEIDON. Isothermal-T setup (50–1000 K prior); two-heterogeneity stellar contamination (T_phot + T_spot + T_fac + f_spot + f_fac); opacity sampling at R = 60,000 (lower than POSEIDON's R = 100,000 due to GPU memory). Validates the main K2-18 b results against retrieval-code choice.
